    ...
    <>
      <div className="fixed inset-0 z-40 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm transition-opacity">
        <div className="bg-white rounded-xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col overflow-hidden animate-in fade-in zoom-in duration-200">
          ...
          {/* Encabezado */}
          {/* RESPONSIVO: p-4 en móvil, shrink-0 para que no scrollee con el contenido */}
          <div className="bg-oltech-black px-4 sm:px-6 py-3 sm:py-4 flex justify-between items-center shrink-0">
            <h2 className="text-lg sm:text-xl font-bold text-white flex items-center space-x-2 min-w-0">
              ...
              <span className="truncate">Auditoría: {ingreso.folio}</span>
            </h2>
          ...
          <div className="overflow-y-auto">
            {mensaje.texto && (
              <div className="px-4 sm:px-6 pt-4 shrink-0">
                 <div className={`p-3 sm:p-4 rounded-md border-l-4 text-sm sm:text-base font-medium ${mensaje.tipo === 'error' ? 'bg-red-50 border-red-500 text-red-700' : 'bg-green-50 border-green-500 text-green-700'}`}>
                    {mensaje.texto}
                 </div>
              </div>
            )}
            ...
            <div className="p-4 sm:p-6 space-y-6 sm:space-y-8">
              ...
              {/* RESPONSIVO: grid-cols-1 en móvil, grid-cols-2 en sm */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 bg-gray-50 p-4 rounded-lg border border-gray-100">
                <div>
                  <p className="text-xs sm:text-sm text-gray-500 mb-1">Pagador / Cliente</p>
                  <p className="font-semibold text-gray-900 text-base sm:text-lg">{ingreso.nombre_quien_paga}</p>
                </div>
                <div>
                  <p className="text-xs sm:text-sm text-gray-500 mb-1">Razón del Ingreso</p>
                  <p className="font-medium text-gray-800 text-sm sm:text-base">{ingreso.razon}</p>
                </div>
                <div>
                  <p className="text-xs sm:text-sm text-gray-500 mb-1">Recibido por (Biomédico)</p>
                  <p className="font-medium text-gray-800 text-sm sm:text-base">{ingreso.biomedico_nombre} {ingreso.biomedico_apellido}</p>
                </div>
                <div>
                  <p className="text-xs sm:text-sm text-gray-500 mb-1">Fecha de Registro</p>
                  <p className="font-medium text-gray-800 text-sm sm:text-base">{new Date(ingreso.fecha).toLocaleString()}</p>
                </div>
              </div>

              {/* Fila 2: Desglose Monetario */}
              <div>
                <h3 className="text-xs sm:text-sm font-bold text-gray-400 uppercase tracking-wider mb-3 border-b pb-1">Desglose Monetario</h3>
                {/* RESPONSIVO: grid-cols-2 en móvil, md:grid-cols-4 */}
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 text-center">
                  <div className="p-3 sm:p-4 border rounded-lg bg-gray-50 flex flex-col justify-center">
                    <p className="text-[10px] sm:text-xs text-gray-500 uppercase tracking-wide">Acordado</p>
                    <p className="text-base sm:text-xl font-bold text-gray-600">{formatearMoneda(ingreso.monto_acordado)}</p>
                  </div>
                  <div className="p-3 sm:p-4 border rounded-lg bg-blue-50 border-blue-200 flex flex-col justify-center">
                    <p className="text-[10px] sm:text-xs text-blue-600 uppercase tracking-wide font-bold">Recibido Físico</p>
                    <p className="text-base sm:text-xl font-bold text-blue-700">{formatearMoneda(ingreso.monto_recibido)}</p>
                  </div>
                  <div className="p-3 sm:p-4 border rounded-lg bg-red-50 border-red-200 flex flex-col justify-center">
                    <p className="text-[10px] sm:text-xs text-red-600 uppercase tracking-wide font-bold">Gastos Ruta</p>
                    <p className="text-base sm:text-xl font-bold text-red-700">-{formatearMoneda(ingreso.monto_gasto || 0)}</p>
                  </div>
                  <div className="p-3 sm:p-4 border rounded-lg bg-green-50 border-green-300 shadow-inner flex flex-col justify-center">
                    <p className="text-[10px] sm:text-xs text-green-700 uppercase tracking-wide font-black">Final a Entregar</p>
                    <p className="text-lg sm:text-2xl font-black text-green-800">{formatearMoneda(ingreso.monto_final || ingreso.monto_recibido)}</p>
                  </div>
                </div>
              </div>

              {/* Fila 3: Observaciones de Gastos */}
              {/* RESPONSIVO: flex-col en móvil para que el botón no aplaste el texto, w-full para el botón */}
              <div className="bg-orange-50 p-4 rounded-lg border border-orange-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-3 md:gap-0">
                <div className="flex-1 w-full">
                  <h3 className="text-xs sm:text-sm font-bold text-orange-800 uppercase tracking-wider mb-1">Observaciones / Gastos</h3>
                  {ingreso.observaciones ? (
                    <p className="text-gray-700 font-medium text-sm sm:text-base">{ingreso.observaciones}</p>
                  ) : (
                    <p className="text-gray-400 italic text-xs sm:text-sm">Sin observaciones ni gastos registrados.</p>
                  )}
                </div>
                ...
                {ingreso.estado_id !== 3 && (rolActual === 'Ventas' || rolActual === 'Sistemas') && (
                  <button 
                    ...
                    className="w-full md:w-auto mt-2 md:mt-0 px-4 py-2 bg-white border border-orange-300 text-orange-700 rounded-lg text-sm font-bold hover:bg-orange-100 transition-colors shadow-sm whitespace-nowrap md:ml-4 flex justify-center items-center"
                  >
              ...
              {/* Fila 4: Evidencias Visuales */}
              <div>
                <h3 className="text-xs sm:text-sm font-bold text-gray-400 uppercase tracking-wider mb-3 border-b pb-1 flex items-center space-x-2">
                  <span>Evidencias Adjuntas</span>
                  <span className="text-[10px] sm:text-xs font-normal bg-gray-100 px-2 py-0.5 rounded-full">(Click para ampliar)</span>
                </h3>
            ...
            {/* Botonera Inferior */}
            {/* RESPONSIVO: flex-col-reverse en móvil, w-full */}
            <div className="p-4 sm:p-6 border-t border-gray-100 bg-gray-50 flex flex-col-reverse sm:flex-row sm:justify-end gap-3 sm:gap-4 shrink-0">
              <button 
                onClick={onClose} 
                disabled={cargando}
                className="w-full sm:w-auto px-6 py-2.5 sm:py-2 rounded-lg font-medium text-gray-600 hover:bg-gray-200 transition-colors flex justify-center items-center"
              >
                Cerrar
              </button>
              ...
              {ingreso.estado_id !== 3 && (rolActual === 'Ventas' || rolActual === 'Sistemas') && (
                <button 
                  ...
                  disabled={cargando}
                  className="w-full sm:w-auto px-8 py-2.5 sm:py-2 bg-green-600 text-white rounded-lg font-bold hover:bg-green-700 transition-colors disabled:opacity-70 flex items-center justify-center space-x-2"
                >
                  {cargando ? (
