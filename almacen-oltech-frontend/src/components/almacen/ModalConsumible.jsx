...
function ModalConsumible({ isOpen, onClose, onGuardado, categoriaId }) { 
  ...
  const handleSubmit = async (e) => {
    ...
    if (incluirCaducidad && !formData.fecha_caducidad.trim()) {
    ...
    try {
      await axios.post('http://localhost:4000/api/almacen/consumibles', {
        ...
        // MODIFICADO: Ya no forzamos formato de fecha, pasamos el string tal cual (en mayúsculas por si escriben "OCT")
        fecha_caducidad: incluirCaducidad ? formData.fecha_caducidad.toUpperCase() : null,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm transition-opacity">
      {/* RESPONSIVO: flex-col y max-h-[90vh] para que el formulario no se desborde con el teclado */}
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg flex flex-col max-h-[90vh] overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        {/* Encabezado */}
        {/* RESPONSIVO: shrink-0 para proteger el header del scroll */}
        <div className="bg-oltech-black px-4 sm:px-6 py-3 sm:py-4 flex justify-between items-center shrink-0">
          <div>
            <h2 className="text-lg sm:text-xl font-bold text-white">Nuevo Insumo</h2>
            <p className="text-oltech-pink text-xs sm:text-sm font-medium">Registro de stock a granel</p>
          </div>
        ...
        {/* Formulario */}
        {/* RESPONSIVO: overflow-y-auto para scroll interno */}
        <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-4 sm:space-y-5 overflow-y-auto">
          ...
          <div className="grid grid-cols-1 gap-4">
            <div>
              ...
              <input 
                ...
                className="w-full px-4 py-2.5 sm:py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-oltech-pink outline-none uppercase font-mono text-base sm:text-sm" 
            ...
            <div>
              ...
              <input 
                ...
                className="w-full px-4 py-2.5 sm:py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-oltech-pink outline-none uppercase text-base sm:text-sm" 
            ...
            <div>
              ...
              <input 
                ...
                className="w-full px-4 py-2.5 sm:py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-oltech-pink outline-none uppercase text-base sm:text-sm italic" 
                placeholder="Ej. MARCA ACME / LÍNEA PREMIUM" 
              />
            </div>
          </div>

          {/* RESPONSIVO: grid-cols-1 en móvil para que no quede apretado, grid-cols-2 en PC */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input 
                ...
                className="w-full px-4 py-2.5 sm:py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-oltech-pink outline-none uppercase bg-white text-base sm:text-sm"
              />
            ...
            <div>
              ...
              <input 
                ...
                className="w-full px-4 py-2.5 sm:py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-oltech-pink outline-none text-center font-bold text-lg" 
              />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              <span className="absolute left-4 top-3 sm:top-2.5 text-gray-500 font-bold">$</span>
              <input 
                type="number" name="precio" step="0.01" min="0"
                value={formData.precio} onChange={handleChange}
                className="w-full pl-8 pr-4 py-2.5 sm:py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-oltech-pink outline-none font-bold text-gray-700 text-base sm:text-sm" 
                placeholder="0.00"
              />
            </div>
          </div>
          ...
          <div className="border-t border-gray-200 pt-4 mt-2">
            <div className="flex flex-col space-y-2">
              <label className="flex items-center space-x-3 cursor-pointer p-2 rounded-lg hover:bg-gray-50 transition-colors border border-transparent hover:border-gray-200">
                <input 
                  ...
                  className="w-5 h-5 text-oltech-pink bg-gray-100 border-gray-300 rounded focus:ring-oltech-pink shrink-0"
                />
            ...
            {(incluirLote || incluirCaducidad) && (
              /* RESPONSIVO: grid-cols-1 en móvil para que los campos adicionales no queden apretados */
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4 bg-pink-50/50 p-4 rounded-lg border border-pink-100 animate-in fade-in slide-in-from-top-2">
                {incluirLote && (
                  <div className={!incluirCaducidad ? "sm:col-span-2" : ""}>
                ...
                {incluirCaducidad && (
                  <div className={!incluirLote ? "sm:col-span-2" : ""}>
                    <label className="block text-sm font-bold text-gray-700 mb-1">Fecha de Caducidad *</label>
                    {/* MODIFICADO: De type="date" a type="text" con placeholder */}
                    <input 
                      type="text" name="fecha_caducidad" required={incluirCaducidad}
                      value={formData.fecha_caducidad} onChange={handleChange}
                      className="w-full px-4 py-2.5 sm:py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-oltech-pink outline-none text-base sm:text-sm uppercase text-gray-700" 
                      placeholder="Ej. 10/26 o OCT 2026"
                    />
                  </div>
                )}
              </div>
          ...
          <div className="pt-4 border-t border-gray-100 flex flex-col-reverse sm:flex-row sm:justify-end gap-3 sm:gap-4 mt-auto">
            <button type="button" onClick={onClose} disabled={cargando} className="w-full sm:w-auto px-6 py-2.5 rounded-lg font-medium text-gray-600 hover:bg-gray-100 transition-colors flex justify-center items-center">
              Cancelar
            </button>
            <button type="submit" disabled={cargando} className="w-full sm:w-auto px-6 py-2.5 bg-oltech-black text-white rounded-lg font-medium hover:bg-gray-800 transition-colors disabled:opacity-70 flex justify-center items-center space-x-2">
              {cargando && (
