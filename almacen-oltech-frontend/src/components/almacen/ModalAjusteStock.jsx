  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm transition-opacity">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        {/* Encabezado Dinámico */}
        <div className={`px-6 py-4 flex justify-between items-center ${esSuma ? 'bg-green-600' : 'bg-red-600'}`}>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 text-center">
            <p className="text-xs font-bold text-gray-500 uppercase">{consumible.codigo_referencia}</p>
            <p className="text-sm font-bold text-gray-800 mt-1">{consumible.nombre}</p>
            <div className="mt-3 inline-block bg-white border border-gray-200 px-4 py-1.5 rounded-full shadow-sm">
              <span className="text-xs text-gray-500 uppercase font-medium mr-2">Stock Actual:</span>
              <span className="text-base font-bold text-oltech-blue">{consumible.cantidad}</span>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex items-center justify-center">
              <input 
                ...
                className={`w-32 px-4 py-3 rounded-lg border focus:outline-none text-center font-bold text-2xl shadow-sm ${
                  esSuma 
          ...
          <div className="pt-4 flex justify-between space-x-3">
            ...
            <button type="submit" disabled={cargando} className={`w-full py-2.5 text-white rounded-lg font-bold shadow-md transition-colors disabled:opacity-70 flex items-center justify-center space-x-2 ${
